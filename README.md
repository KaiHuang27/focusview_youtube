...
- Outside the player, page scrolling remains native so Safari trackpad and mouse wheel scrolling remains smooth, including while zoom mode is on. The global Safari wheel fallback is attached only while the pointer is inside the player, preserving reliable player zoom when Safari or YouTube routes the event through another overlay. Wheel-time UI updates remain batched to keep non-fullscreen controls responsive.
