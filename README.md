...
- Outside the player, page scrolling remains native so Safari trackpad and mouse wheel scrolling remains smooth, including while zoom mode is on. The global Safari wheel fallback is attached only while the pointer is inside the player, with cached-pointer recovery after player replacement and fullscreen changes so Safari cannot leak wheel input into native page scrolling. Wheel-time UI updates remain batched to keep non-fullscreen controls responsive.
