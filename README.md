...
- YouTube-style transform menu with a zoom slider from 100% to 500%.
...
The zoom button is placed in YouTube's native right-side control bar, at the left edge of the right control group. It uses YouTube's native `ytp-button` sizing, a centered outline icon, and a small percentage badge only while the menu is open or zoom is not 100%. Click it to open or close the transform menu, or double-click it to reset zoom to 100%. If YouTube's native controls are unavailable, the extension falls back to a floating top-right button.
