...
The transform menu opens from the settings button centered below the top-right position map during recent Pan activity. It follows YouTube's settings-menu density: rounded gray panel, compact centered text rows, a top `- / slider / +` zoom control, and a small red `Reset` action in the top-right corner.
