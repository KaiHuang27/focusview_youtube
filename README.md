...
- Scroll to zoom videos from `100%` to `500%` around the mouse pointer, with each nonzero wheel or trackpad event capped at about fifteen percent and animated by at least one percent unless the zoom limit is reached.
...
6. Click the FocusView toolbar button and confirm the popup opens with the FocusView name, concise YouTube tagline, and zoom mode shortcut hint.
...
6. Click the Safari toolbar FocusView button and confirm the popup responds with the FocusView name, concise YouTube tagline, and zoom mode shortcut hint.
