...
test("applyWheelZoomDelta scales zoom from wheel pixel delta", () => {
  assert.equal(applyWheelZoomDelta(100, { deltaY: -10, deltaMode: 0 }), 100.702);
  assert.equal(applyWheelZoomDelta(100, { deltaY: 10, deltaMode: 0 }), 100);
  assert.equal(applyWheelZoomDelta(200, { deltaY: 10, deltaMode: 0 }), 198.605);
});

test("applyWheelZoomDelta preserves fractional trackpad input", () => {
  assert.equal(applyWheelZoomDelta(150, { deltaY: -1, deltaMode: 0 }), 150.105);
  assert.equal(applyWheelZoomDelta(150, { deltaY: 1, deltaMode: 0 }), 149.895);
...
test("applyWheelZoomDelta makes larger wheel deltas zoom faster", () => {
  assert.equal(applyWheelZoomDelta(100, { deltaY: -40, deltaMode: 0 }), 102.84);
  assert.equal(applyWheelZoomDelta(100, { deltaY: -80, deltaMode: 0 }), 105.76);
  assert.equal(applyWheelZoomDelta(100, { deltaY: -120, deltaMode: 0 }), 107.251);
});

test("applyWheelZoomDelta normalizes delta modes and clamps extreme events", () => {
  assert.equal(applyWheelZoomDelta(100, { deltaY: -3, deltaMode: 1 }), 103.417);
  assert.equal(applyWheelZoomDelta(100, { deltaY: -1, deltaMode: 2 }), 107.251);
...
test("rapid wheel deltas accumulate without discarding unfinished input", () => {
  ...
  assert.ok(targetZoom > 107.2 && targetZoom < 107.3);

  targetZoom = applyWheelZoomDelta(targetZoom, { deltaY: 25, deltaMode: 0 });
  targetZoom = applyWheelZoomDelta(targetZoom, { deltaY: 25, deltaMode: 0 });
  assert.ok(targetZoom > 103.5 && targetZoom < 103.6);
});
