...
4. The zoom trigger renders as a fixed-size native `ytp-button` magnifier icon at rest, then switches to centered percentage text while Pan mode is on or zoom is not 100%. The toolbar slot stays 48px wide in both icon and text modes. Hover and active backgrounds use the same 40px circular internal background element instead of a pseudo-element, so YouTube's native `ytp-button` styles cannot override the active state. Only Pan mode adds the active background and heavier text; zoomed-but-Pan-off keeps the percentage text without active styling.
...
- Confirm Pan on shows an active 40px circular background with the same size, shape, and position as hover but slightly lower opacity, plus heavier percentage text, while Pan off with zoom above 100% shows percentage text without the active background.
