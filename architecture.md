...
- `src/overlay.css`: native-control-bar zoom trigger using YouTube's `ytp-button` sizing, Roboto/Arial typography, centered percentage text, floating fallback trigger, compact YouTube-settings-style gray menu, toggle, segment, YouTube-style slider track, and position-map styling.
