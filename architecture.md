...
- `src/content.js`: detects the YouTube player, inserts the zoom trigger into `.ytp-right-controls` when available, renders the YouTube-style menu with row icons, updates transform state, renders the position map, handles pan dragging, handles Pan-mode wheel zoom, handles the Pan keyboard shortcut, reapplies transforms after fullscreen/style mutations, and resets state on YouTube SPA navigation.
...
5. Clicking the zoom trigger opens a YouTube-settings-style gray menu with Zoom, Rotation, Mirror H, Mirror V, Pan, and Reset rows. Each row has a left icon, center label, and right-aligned value or control. Double-clicking the trigger resets only zoom and pan to the centered 100% view.
...
- Confirm clicking the zoom button opens and closes the YouTube-style transform menu with gray rounded panel, left icons, and right-aligned controls.
