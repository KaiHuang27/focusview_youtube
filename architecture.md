...
- `src/content.js` owns the YouTube player integration, toolbar UI, pointer and wheel interactions, Fill Screen action, transform application, and player state reset triggers.
- `src/transform-state.js` contains the shared transform state helpers used by the content script and tests, including viewport-centered zoom for controls, cursor-centered zoom for wheel input, cover zoom calculation for Fill Screen, and full transform reset state.
