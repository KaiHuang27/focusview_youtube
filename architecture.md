...
4. The zoom trigger renders as a native-sized `ytp-button` magnifier icon at rest, then switches to centered percentage text while Pan mode is on or zoom is not 100%. Its hover and active backgrounds use the same internal background element instead of a pseudo-element, so YouTube's native `ytp-button` styles cannot override the active state. Only Pan mode adds the active background and heavier text; zoomed-but-Pan-off keeps the percentage text without active styling.
