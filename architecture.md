...
- `src/content.js` owns the YouTube player integration, toolbar UI, pointer and wheel interactions, Zoom mode preservation of native single-click play/pause, drag-time restoration from YouTube's 2x hold speed, Fill action, reset and zoom mode tooltip wording, transform application, and player state reset triggers.
...
- `src/transform-state.test.js` verifies transform state behavior, Zoom mode gesture guards, and drag-time playback-rate restoration with Node's built-in test runner.
