...
- `src/overlay.css`: native-control-bar zoom trigger using YouTube's `ytp-button` sizing, magnifier icon, a dedicated trigger background layer, compact conditional percentage text, matching text-mode hover/active background size, Roboto/Arial typography, floating fallback trigger, top-right settings button, compact YouTube-settings-style gray menu, toggle, segment, YouTube-style slider track, and position-map styling.
...
4. The zoom trigger renders as a native-sized `ytp-button` magnifier icon at rest, then switches to centered percentage text while Pan mode is on or zoom is not 100%. In text mode, the native toolbar slot expands and hover/active backgrounds use the same internal background element and dimensions instead of a pseudo-element, so YouTube's native `ytp-button` styles cannot override the active state. Only Pan mode adds the active background and heavier text; zoomed-but-Pan-off keeps the percentage text without active styling.
