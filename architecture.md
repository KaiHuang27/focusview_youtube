...
6. During Pan dragging or Pan-mode wheel zoom, the top-right position map and centered settings button appear. Mouse movement keeps them visible briefly, and they hide after the activity delay. Clicking the settings button opens a compact YouTube-settings-style gray menu anchored beside that settings area. The top section shows the zoom scale with dense `- / slider / +` controls and a small red Reset action in the top-right corner; the remaining rows use compact centered text labels and right-aligned controls.
