...
9. `createViewportCenteredZoomState` scales pan offsets by the zoom ratio so zoom changes preserve the Source Video content currently shown at the Player Viewport center. The menu slider, `-` / `+` buttons, manual input, and Pan-mode wheel share this path. Player-level capture wheel events are intercepted before YouTube can handle them, update zoom in 5% steps, and clamp it to 100%-500%. Wheel events that start inside the menu are blocked without changing zoom.
