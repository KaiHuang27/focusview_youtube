...
- `src/transform-state.test.js`: Node test coverage for reset state, zoom scale conversion, rotation fit scaling, rotated Source Map geometry, zoom and viewport-aware pan clamping, Pan-mode wheel interception, transient viewport-control visibility, Pan shortcut detection, transform reapply detection, viewport-map math, rotation validation, and mirror composition.
...
6. When Pan turns on, during Pan dragging, or during Pan-mode wheel zoom, the top-right position map and centered settings button appear. Mouse movement inside the player keeps them visible briefly or shows them again after they hide, and they hide after the activity delay unless the transform menu is open. Clicking the settings button opens a compact YouTube-settings-style gray menu `8px` below the button. The menu contains zoom, rotation, horizontal mirror, and reset controls; Pan remains controlled by the toolbar trigger and shortcut.
7. `Alt/Option + Shift + Z` turns Zoom mode on or off at window/document capture time, but only outside editable fields and without `Ctrl` or `Cmd` modifiers.
...
9. `createViewportCenteredZoomState` scales pan offsets by the zoom ratio so zoom changes preserve the Source Video content currently shown at the Player Viewport center. The menu slider, slider keyboard controls, `-` / `+` buttons, manual input, and Pan-mode wheel share this path. Slider arrow keys move in 5% steps, `Home` selects 100%, and `End` selects 500%. Player-level capture wheel events are intercepted before YouTube can handle them, update zoom in 5% steps, and clamp it to 100%-500%. Wheel events that start inside the menu are blocked without changing zoom.
10. `getRotationFitScale` contain-fits the Source Video into the Player Viewport, then computes the fit scale for the rotated bounding box before user zoom is applied. Landscape sources shrink when rotated to portrait orientation, while portrait sources can expand when rotated to landscape orientation.
11. `createDisplayedSourceSize` swaps Source Video width and height for 90/270-degree rotations. `clampPanStateToViewport` bounds pan to that rotated Source Video content after it is contain-fit into the current Player Viewport. Directions that are still black-border-only stay centered until zoom fills that side of the viewport.
...
13. An active transform is applied through an extension stylesheet rule with `!important`, so YouTube fullscreen inline-style rewrites do not briefly override the active zoom. The cached rule is only rewritten when its CSS changes and is cleared when the state returns to the default transform.
14. `createViewportMapSize` sizes the Source Map from the rotated Source Video aspect ratio within min/max bounds, `createViewportIndicator` maps the current Player Viewport, zoom, and pan through the same rotated contain-fit geometry into a normalized indicator that may extend outside the Source Map, and `createViewportOverlayLayout` positions the Source Map plus settings button from a 100% zoom anchor indicator so zooming and panning do not move the overlay.
15. URL, player, or fullscreen-related layout changes trigger `sync`; the current transform, clamped pan, and position controls are reapplied or reparented to the current player root. Leaving a watch page or switching the active video runs centralized cleanup for video listeners, observers, pending gestures, click suppression, and document-level slider-drag listeners.
...
- Confirm moving the pointer inside the player shows the position map and settings button again while Pan mode remains active.
...
- Confirm the top-right Source Map appears below the YouTube title area while dragging or wheel-zooming in Pan mode, uses the displayed Source Video aspect ratio, and swaps width and height for 90/270-degree rotations. Confirm its transparent white-outline Viewport Indicator uses the same rotated geometry and the current Player Viewport ratio. Confirm indicator overflow communicates letterbox or pillarbox space without visually merging into Source Map content. Confirm zooming or panning moves only the indicator, while the Source Map and settings button stay fixed for the current Player Viewport size.
...
- Confirm the settings button centered below the top-right position map uses a native-style SVG gear icon with low-emphasis default background, subtle hover, and active state when the menu is open. Confirm clicking it opens and closes the YouTube-style transform menu `8px` below the settings button, with dark translucent rounded panel, native-like smaller gray controls, rounded inset row hover states, compact top zoom control with white progress and gray remaining track, red top-right Reset action, text rows, and right-aligned controls that stay inside the panel.
- Confirm menu Zoom, Rotation, horizontal Mirror, and Reset. Confirm the Zoom value displays as a percentage, selects all text when clicked, accepts manual numeric input, and blocks YouTube number-key seek shortcuts while focused. Confirm clicking either the Mirror switch or the full Mirror row toggles mirror once. Confirm the menu does not show Mirror V or Pan rows.
- Confirm `Alt/Option + Shift + Z` turns Zoom mode on or off while focus is on the video page, and does nothing while typing in YouTube search or comments.
- Confirm slider changes and Pan-mode wheel zoom update the zoom button text.
- Confirm slider, `-` / `+`, manual input, and Pan-mode wheel zoom preserve the content currently shown at the Player Viewport center after the video has been panned. Confirm returning to 100% recenters the video.
- Confirm pressing and holding the wider zoom slider hit area, not only the thin track, starts dragging and keeps dragging until pointer release, even when the cursor moves above or below the track. Confirm closing the menu, leaving the watch page, or switching videos during a drag does not leave a stale drag listener.
- Confirm focusing the Zoom slider allows arrow-key changes in 5% steps, `Home` for 100%, and `End` for 500%, without triggering YouTube native controls.
- Confirm periodic YouTube DOM checks preserve the mounted menu while editing the Zoom value or dragging the slider, including when YouTube rebuilds its native control bar and the extension remounts the toolbar button.
...
- Confirm 90/270-degree rotation fits the rotated video inside the player frame at 100% zoom without cropping the rotated top/bottom or left/right edges. Confirm a portrait Source Video expands correctly after rotating into landscape orientation instead of shrinking again.
