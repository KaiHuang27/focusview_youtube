...
- `src/overlay.css`: native-control-bar zoom trigger with a YouTube-like SVG icon, conditional percentage badge, hover/active feedback, floating fallback trigger, YouTube-style dark menu, toggle, segment, slider, and position-map styling.
...
4. The zoom trigger renders as a 48px icon button. It shows a small percentage badge only when the menu is open or zoom is not 100%.
...
- Confirm the zoom button renders as a native-looking icon button, with no large `100%` label at default 100% and menu closed.
- Confirm hover uses a subtle circular background and menu-open active state uses a slightly stronger circular background.
