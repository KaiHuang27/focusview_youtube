...
- Confirm the top-right Source Map appears below the YouTube title area while dragging or wheel-zooming in Pan mode, uses the Source Video aspect ratio, and shows a Viewport Indicator using the current Player Viewport ratio. Confirm zooming or panning moves only the indicator, while the Source Map and settings button stay fixed for the current Player Viewport size.
