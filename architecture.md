...
- `src/overlay.css`: native-control-bar zoom trigger using compact fixed YouTube-style pill sizing, magnifier icon, native `ytp-button` hover styling, a dedicated fixed pill active background layer, compact conditional percentage text, Roboto/Arial typography, floating fallback trigger, top-right settings button, compact YouTube-settings-style gray menu, toggle, segment, YouTube-style slider track, and position-map styling.
...
4. The zoom trigger renders as a compact fixed-size native-style pill with a magnifier icon at rest, then switches to centered percentage text while Pan mode is on or zoom is not 100%. The toolbar slot and internal active background keep the same dimensions in both icon and text modes, including narrow viewports. Hover is left to YouTube's native `ytp-button` styling; while hovered, the extension hides its active background layer so the native hover is not double-stacked. Only Pan mode adds the active background and heavier text; zoomed-but-Pan-off keeps the percentage text without active styling.
