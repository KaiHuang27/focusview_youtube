...
- Confirm the top-right Source Map appears below the YouTube title area while dragging or wheel-zooming in Pan mode, uses the displayed Source Video aspect ratio, and swaps width and height for 90/270-degree rotations. Confirm its Viewport Indicator uses the same rotated geometry and the current Player Viewport ratio. Confirm zooming or panning moves only the indicator, while the Source Map and settings button stay fixed for the current Player Viewport size.
