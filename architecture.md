...
- `src/content.js` owns the YouTube player integration, toolbar UI, pointer and wheel interactions, early Zoom mode interception of YouTube's 2x hold gesture, Fill action, reset and zoom mode tooltip wording, transform application, and player state reset triggers.
...
- `src/transform-state.test.js` verifies transform state behavior and Zoom mode gesture guards with Node's built-in test runner.
