...
- `src/content.js` owns the YouTube player integration, toolbar UI, pointer and wheel interactions, Zoom mode-only player wheel blocking, pointer-presence-scoped window/document fallback for Safari with cached-pointer recovery across player replacement and fullscreen changes, and batched wheel UI updates for smooth Safari page scrolling, proportional wheel zoom animation, Zoom mode preservation of native single-click play/pause, drag-start cancellation of YouTube's 2x hold gesture, drag-time playback-rate restoration, Fill action, reset and zoom mode tooltip wording, transform application, and player state reset triggers.
- `src/transform-state.js` contains the shared transform state helpers used by the content script and tests, including viewport-centered zoom for controls, about-fifteen-percent capped delta-aware cursor-centered zoom for wheel input, player bounds and pointer-presence-aware fallback listener decisions, player-bounds wheel hit testing, proportional wheel zoom animation steps, cover zoom calculation for Fill, and full transform reset state.
...
- `src/transform-state.test.js` verifies transform state behavior, capped wheel delta zoom scaling, minimum one-percent wheel response, Zoom mode-only player blocking and pointer-presence-scoped fallback decisions, player-bounds wheel hit testing, proportional wheel zoom animation steps, Zoom mode gesture guards, drag-start hold cancellation, and drag-time playback-rate restoration with Node's built-in test runner.
...
- `src/product-wording.test.js` verifies store-specific manifest metadata, user-facing control labels, wheel zoom animation wiring, pointer enter/move/leave-scoped window/document fallback and geometry-resync wiring, batched wheel UI updates, and Safari setup wording across Chrome and Safari resources.
