...
8. `createViewportFrame` maps zoom and pan into a normalized visible rectangle for the top-left position map.
9. URL, player, or fullscreen-related layout changes trigger `sync`; the current transform, clamped pan, and position map are reapplied to the same video element.
10. When the video key changes, state resets to defaults.
